...
# header). Might revisit later. CBA atm.
...
class DataWiz:
    ...
    def process(self):
        ...
        if isinstance(self.array, numpy.ndarray):
            ...
            for column in xrange(
                    0, len(self.array[0, 0:])):  # Test each column
                ...
                if isinstance(
                        self.array[
                            0, column], numpy.string_) and sum(
                        self.accum) < 41 and sum(
                        self.accum) > 0:
            ...
            for column in xrange(0, len(self.array[0, 0:])):
                if (self.col_is_categorical[column] and self.col_is_datetime[
                        column] == False):  # if it's categorical but not a date
                    ...
                    if float(no_of_unique) / float(len(self.array)) > 0.25:
                        # ... also, even if we accidentally rule out a legitimate feature, the metric being > 0.25  would probably be a feature that'll cause overfitting
    ...
    ##########################################################################
        if isinstance(self.array, pandas.core.frame.DataFrame):
            for column in xrange(
                    0, len(self.array.columns)):  # Test each column
                # initialize an array of 40 valid indexes to randomly sample in
                # a given column. We reset the first value of the array to 0 to
                # test the potential header row
                test_value_types = [randint(1, len(self.array) - 1)
            ...
            for index, column in enumerate(self.array.columns):
                if (self.col_is_categorical[
                        index] and self.col_is_datetime[index] == False):
                            # convert to number labes using LabelEncode
                    encoder = preprocessing.LabelEncoder()
    ...
    def process_test(self):
        ...
        if isinstance(self.array_test, numpy.ndarray):
            ...
            for column in xrange(0, len(X_test[0, 0:])):
                ...
                if (not isinstance(encoders_local[column], str) and is_dt_local[
                        column] == False):  # If column is categorical but also a datetime, don't convert it
                    # convert to number labels using LabelEncode
                    # print column
                    if self.advanced_ops:  # remove leading or trailing spaces
                        X_test[:, column] = numpy.char.strip(X_test[:, column])
                    # output of encoder.transform is a numpy.ndarray, FYI
                    X_test[:, column] = encoders_local[
                        column].transform(X_test[:, column], True)
                if self.dt_convert == 1:
        ...
        if isinstance(self.array_test, pandas.core.frame.DataFrame):
            ...
            # Handle missing values
            if (self.missing_vals == 'fill' or self.missing_vals ==
                    'drop'):  # Missing values shouldn't be dropped in the test set
            ...
            for index, column in enumerate(X_test.columns):
                ...
                if not isinstance(
                        ...
                        str) and is_dt_local[index] == False:
                    if self.advanced_ops:
